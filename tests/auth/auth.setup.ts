...
setup('authenticate as tool-admin', async ({page}) => {
    await page.goto(
        'login_check?user=test-tool-admin&expires=33297865200&hash=hx-zSxVnaeiVWLmmE-1vdiiSXU_WOZfldT7vlDfH6a0~RVmHT5SCRm0uA0q9V-ghnDGpWORxvhFBGeQ9yJXs2T4~'
    );
...
setup('authenticate as user', async ({page}) => {
    ...
    await page.goto(
        'login_check?user=test-testovic&expires=33297865200&hash=uNdTW6_tQ9wDhe-xg27iumK-anWNx6rCntdQrIJYD7Q~gGd5DWPd2nsurngPvG9AYTz-FDuB1vaqUqcMCoQgoyI~'
    );
    await page.waitForURL('');
    // Alternatively, you can wait until the page reaches a state where all cookies are set.
    // await expect(page.getByRole('button', { name: 'View profile and more' })).toBeVisible();
